		...
		<div class="main">
			...
			<div class="main-content">
							...
							<tbody>
								<tr>
									<td>标题 <input name="title" value="${title}"
										...
										type="hidden" name="lyren" value="${student.stno}" /> <input
				...
				<div class="action-details">
					<span id="btnCheckAll" class="button">选择</span>
					 <span class="button" id="btnDelete" >删除
					</span>
				</div>
					...
					<tbody>
						...
						<%
							if (request.getAttribute("listleaveword") != null) {
								List<Leaveword> listleaveword = (List<Leaveword>) request
										.getAttribute("listleaveword");
								for (Leaveword temleaveword : listleaveword) {
						%>
						<tr>
							<td>&nbsp<input id="chk<%=temleaveword.getId()%>"
								class="check" name="ids"
								type="checkbox" value='<%=temleaveword.getId()%>'>
							</td>
							...
							<td>
								<%
									if (temleaveword.getStatus() == 1) {
								%> <a class="orange-href"
								href="${pageContext.request.contextPath}/admin/leavewordmanager.do?actiontype=load&id=<%=temleaveword.getId()%>&tno=<%=temleaveword.getReplyren()%>&forwardurl=/e/huiyuan/leavewordadd.jsp&seedid=205">修改</a>
								<%
									}
								%>
								 <%
							 ...
							 %> 
							   <a class="orange-href"
															href="${pageContext.request.contextPath}/e/huiyuan/leaveworddetails.jsp?id=<%=temleaveword.getId()%>&seedid=205">查看</a> 
							<%
							 	}
							 %>
							</td>
