...
<html>
 <%
	...
	List<Teacher> teacherlist = teacherSrv.getPageEntitys(filter,pageindex, pagesize);
		       ...
		       <c:forEach var="teacher" items="${listTeacher}">
		        <li>
		             <div class="item">
		              ...
		              <div class="img">
					   <a href="${pageContext.request.contextPath}/e/teacherinfo.jsp?id=${teacher.id}" >
                           <img src="${teacher.xiangpian}" />
						 </a>
		              </div>
		               <div class="name">${teacher.name}-${teacher.tno}</div>
		               <div class="price"></div>
		               <div class="discount">心理疏导</div>
		           </div>
		        </li>
		        </c:forEach>
		     </ul>
...
没有找到相关教师信息
						...
						<div class="clear"></div>
				<form id="teachersearchForm" method="post" action="${pageContext.request.contextPath}/e/teacherlist.jsp">
<input type="hidden" name="actiontype" value="get" />
                      <input type="hidden" name="forwardurl" value="/e/teacherlist.jsp" />
                 </form>
		<daowen:pager id="pager1"  attcheform="teachersearchForm" />
	</div>
